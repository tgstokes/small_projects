...
def get_num_attempts():
    # get user input for number of incorrect attempts per game
    while True:
        num_attempts = input(r"How many incorrect attempts do you want to have? [1-25]")
        try:
...
def get_minimum_word_length():
    # get user input for minimum word length for game
    while True:
        minimum_word_length = input(r"What minimum word length would you like? [3-16]")
        try:
            minimum_word_length = int(minimum_word_length)
            if 3 <= minimum_word_length <= 16:
                return minimum_word_length
            else: print('{0} is not between 3 and 16'.format(minimum_word_length))
        except ValueError:
            print('{0} is not an integer between 1 and 25'.format(minimum_word_length))
